...
class PlaceReminder {
  ...
  factory PlaceReminder.fromFirestore(DocumentSnapshot doc) {
    ...
    return PlaceReminder(
      ...
      placeTitle: d['title'] ?? d['placeTitle'] ?? '',   // place_details writes 'title'
      note: d['note'] ?? '',
...
class RemindersScreen extends StatelessWidget {
  ...
  Future<void> _deleteReminder(
      BuildContext context, String placeId, UserRole role) async {
    final uid = _uid;
    if (uid == null) return;
    ...
    await FirebaseFirestore.instance
        .collection('users')
        .doc(uid)
        .set({'limits': {'remindersUsed': FieldValue.increment(-1)}},
                        ...
                        Expanded(
                          child: ListView.builder(
                            ...
                            itemBuilder: (context, i) {
                              final reminder = PlaceReminder.fromFirestore(docs[i]);
                              return _ReminderCard(
                                reminder: reminder,
                                onDelete: () => _deleteReminder(
                                    context, reminder.placeId, role),
                              );
                            },
                          ),
                        ),
...
Future<void> addReminder({
  ...
  if (context.mounted) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
          content: Text('Reminder set for "$placeTitle". We\'ll notify you nearby!')),
    );
  }
}